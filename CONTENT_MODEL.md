...
Reference tables — Appendix A "Conjugation at a Glance" **plus every in-chapter formation/function table** (chapters 1, 2, 5, 6, 7, 10, 12, 19, 21, 22, 23, 24 — 29 tables total) — are transcribed verbatim into `src/data/grammar-tables.ts` (`GrammarTable { columns, rows, note?, chapterIds, pointIds?, reference? }`). Tables with `pointIds` render inside their points' teach cards, exactly where the book teaches them; `reference` tables also appear in the hub's "Conjugation at a glance". Appendix B's Particle Quick Reference lives in `src/data/grammar-particles.ts` (hub reference), and each chapter's `commonMistake` + 3-question `miniCheck` render as the lesson's closing wrap-up step. Both kana datasets also carry the books' practice-reading words (~100 total) as per-kana `examples`, feeding the mnemonic cards, the chart, and Word Builder.

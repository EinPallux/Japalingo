...
/** A clean, horizontally-scrollable reference table. The first column reads as
 *  a row label; the rest are Japanese forms (font-jp). `hideTitle` is for
 *  embedding inside a teach card whose heading already names the topic. */
export function GrammarTableCard({
  table,
  className,
  hideTitle = false,
}: {
  ...
  return (
    <div className={cn("flex w-full max-w-lg flex-col gap-3", className)}>
      {hideTitle ? null : <h2 className="font-display text-xl font-bold text-ink">{table.title}</h2>}

...
**Current status:** Phase 0 **COMPLETE**. Phase 1 (Landing page) **BUILT**. Phase 2 (Learning platform) **IN PROGRESS** — playable: onboarding, **Hiragana + Katakana** tracks with a switcher, the lesson player (Mnemonic Story, Quick Match, Kana Drill), **five game modes** (Romaji Rush, Kana Rain, Kana Match, Ear Training, Word Builder), a **real SRS review scheduler** (per-kana due-dates; "N due" surfacing), a **MARU-style Free Drill** (train any kana rows, weakness-prioritized), a **gojūon reference chart**, a **"Meet the Sounds" beginner primer**, **daily quests**, a **two-currency shop** (Coins + Gems → Hoshi cosmetics, Streak Freezes, XP Boosts), a **profile** with a mastery grid + badges + Hoshi outfit, a **"Your Journey" insights view**, a **Settings** screen (theme, daily goal, sound, speech speed), and XP/streak/coins/gems/mastery with local save + audio. A full logical-correctness audit has been applied (games quiz only met kana, clean distractors, honest accuracy/streak/quest bookkeeping, gentler onboarding → primer → first lesson). The app is now an **installable PWA** (web manifest + icons, iOS home-screen meta + safe-area insets, a service worker for offline, and an iOS "Add to Home Screen" hint). A second audit + polish pass added a **decluttered dashboard** (2×2 quick actions, Arcade hub at `/learn/games`), a **Duolingo-level animation layer** (confetti results, XP count-up, pop/shake answer feedback, sliding exercises — all reduced-motion-safe), and DST-safe streak day-math. A progression + personality pass then added **Speed Review checkpoints** (timed per-unit run → gold crown, at `/learn/review/[unitId]`), **local high-score boards** (per-track bests + "new record!" celebration for Kana Rain & Romaji Rush), and an **in-lesson Hoshi coach** (cheers on 3+ streaks, encourages after misses). Two **content expansions** landed, all transcribed from the books' variation pages: **dakuten & han-dakuten** (50 kana — G/Z/D/B ゛ + P ゜) and then the **combination kana (yōon, 66)** plus **ヴ (vu)**, the **extended combination katakana** (22 foreign-sound combos: ファ/ヴァ/ツァ/ティ/シェ…), and **concept lessons** for the small っ and the long vowel ー — all added to the path after the basics, into the chart, games, drill, SRS, and Speed Review. That completes both Tofugu books' kana content (**104 hiragana / 127 katakana**, 231 total). A **Vocabulary track** then landed (content-gated to `database/Vocabulary_of_JLPT_N5.pdf`): **802 JLPT N5 words** transcribed to a typed dataset, taught **kana-first** (kanji is optional reference, never required) via **frequency-ordered decks** at `/learn/vocab` — a gated deck path (greetings first, then most-common-words-first core), a gamified deck lesson player (word↔meaning + listening exercises, reusing the pop/shake feedback, Hoshi coach, confetti, XP/coins/gems), and per-word SRS review. A **Grammar track** then landed (content-gated to `database/Japalingo_Japanese_Grammar_for_Absolute_Beginners.pdf`, a beginner adaptation based on Tae Kim's guide): **24 chapters / 130 points / 108 example sentences + the 50 core patterns**, transcribed to a typed dataset, taught in the book's own researched beginner order (three parts) at `/learn/grammar` — a gated chapter path, a gamified lesson player (teach cards, then **translate**, **say-it-in-Japanese**, and a tap-to-assemble **build-the-sentence** game, all **kana-first**), per-point SRS review, and a collapsible 50-pattern reference. **Next: full app-UI i18n (EN/DE) and the Dexie/IndexedDB persistence swap.**
...
| Future | Content-gated | ~~Vocabulary~~ (**shipped** — JLPT N5) · ~~Grammar~~ (**shipped** — beginner, from `/database`), kanji, sentences/listening, recorded audio, accounts + cloud sync + leagues/friends, PWA polish. Build ONLY when `/database` grows. |
